/* URI - 2992: https://www.urionlinejudge.com.br/judge/pt/problems/view/2992 */
...
VALUES(9999, 'Jose Sampaio', 'R. Z', '2006-06-06T00:00:00Z', 1, 1, 12, NULL),
...
SELECT 
	DISTINCT ON (departamento.nome) departamento.nome AS departamento,
	average_division_salary.division AS divisao,
	MAX(average_division_salary.media) AS media
FROM
(
	SELECT 
		divisao.nome AS division,
		divisao.cod_dep AS cod_dep,
		ROUND(AVG(employee_salary.salary - employee_discount.discount), 2) AS media
	FROM
	(
		SELECT 
			empregado.matr AS matr,
			empregado.lotacao_div AS lotacao_div,
			SUM(COALESCE(vencimento.valor, 0)) AS salary 
		FROM emp_venc
		JOIN vencimento ON vencimento.cod_venc = emp_venc.cod_venc
		RIGHT JOIN empregado ON empregado.matr = emp_venc.matr
		GROUP BY empregado.matr, lotacao_div
		ORDER BY empregado.matr
	) employee_salary
	JOIN
	(
		SELECT 
			empregado.matr AS matr, 
			SUM(COALESCE(desconto.valor, 0)) AS discount
		FROM emp_desc
	...
	ON employee_salary.matr = employee_discount.matr
	JOIN divisao ON employee_salary.lotacao_div = divisao.cod_divisao
	GROUP BY division, cod_dep
) average_division_salary
JOIN departamento ON departamento.cod_dep = average_division_salary.cod_dep
GROUP BY departamento.nome, divisao
ORDER BY departamento.nome DESC, media DESC;
