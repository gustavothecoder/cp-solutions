-- URI 2997: https://www.urionlinejudge.com.br/judge/en/problems/view/2997
...
SELECT 
	...
	COALESCE("Salario Bruto", 0),
	COALESCE("Total Desconto", 0),
	CASE COALESCE("Salario Bruto", 0) - COALESCE("Total Desconto", 0)
		WHEN 0 THEN COALESCE("Salario Bruto", 0) - COALESCE("Total Desconto", 0)
		ELSE ROUND(COALESCE("Salario Bruto", 0) - COALESCE("Total Desconto", 0), 2)
...
LEFT JOIN (	
	SELECT 
		matr, 
		ROUND(SUM(vencimento.valor), 2) AS "Salario Bruto" 
	FROM emp_venc
	JOIN vencimento ON vencimento.cod_venc = emp_venc.cod_venc
	GROUP BY matr
) gross_salary ON gross_salary.matr = empregado.matr
LEFT JOIN (
	SELECT 
		matr, 
		ROUND(SUM(desconto.valor), 2) AS "Total Desconto" 
	FROM emp_desc
	JOIN desconto ON desconto.cod_desc = emp_desc.cod_desc
	GROUP BY matr
) total_discount ON total_discount.matr = empregado.matr
